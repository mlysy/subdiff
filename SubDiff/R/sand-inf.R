#' @title Downsampling Inference using Composite Likelihood and Sandwich Estimator
#' @description 
...
sandwich.infer <- function(Yt, dt, acf, rate = 1, ConfInt = FALSE) {
  ...
  est <- optimize(f = sandwich.infer.prof, interval = c(0, 2), Yt = Yt, dt = dt, acf = acf, 
               rate = rate, control = list(fnscale = -1))$par
  if(ConfInt) {
    err.bar <- sand.estimator(func = sandwich.infer.prof, x = est, Yt = Yt, dt = dt, acf = acf, 
                              rate = rate)
