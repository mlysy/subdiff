...
output: 
  rmarkdown::html_vignette:
    toc: yes
vignette: >
  %\VignetteIndexEntry{msd_conventions}
...
D <- .43
mu <- runif(d) # drift per coordinate
# simulation setup
...
tibble(t = 1:nrow(msd_hat) * dt,
       direct = 2 * msd_hat[,"direct"],
       nodrift = 2 * msd_hat[,"nodrift"],
       true = 2*ndim * D * t) %>%
...
- A version of least-squares (`LS`).  There is no unique definition of the LS estimator as it depends among other things on the method of drift correction and the set of timepoints used in the regression.  Here we use a linear drift subtraction and timepoints $t = \dt \cdot (1, 2, 5, 10, 20, 50, 100, 200, 500)$.

- MLE of fractional Brownian motion (`fBM`).  Drift correction in this and the subsequent MLE estimators is linear.

<!-- - MLE of fBM + white noise floor (`fSN`). -->

- MLE with fBM + Savin-Doyle noise model (`fSD`).  This accounts for static noise via a white noise floor and dynamic errors due to particle movement during the camera exposure time for each frame.  It's identical to fSN when there is no dynamic error (camera exposure time = 0).

- MLE with fBM + MA(1) noise (`fMA`).  This is one of the models proposed in our AOAS paper.

- MLE with fBM + ARMA(1,1) noise (`fARMA`), another model proposed in our AOAS paper.
...
ad_fit[2,] <- exp(ad_fit[2,])/2 # new convention for D
...
This just confirms that the estimators remain consistent with simulated fBM.

```{r fbm_fit, fig.width = 7, fig.height = 3.5, out.width = "100%"}
ndim <- 2 # number of trajectory dimensions
# true parameter values
D <- .43
