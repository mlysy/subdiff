...
1.  Calculate the empirical MSD for each trajectory, defined as
    ...
    \begin{equation}
	\widehat{\msd}_{\XX}(t_n) = \frac 1 {\ndim(N-n+1)} \sum_{h=0}^{N-n} \Vert \XX_{h+n}-\XX_{h}\Vert^2.
	(\#eq:msdemp)
	\end{equation}
...
msd_hat <- lapply(ids, function(id) {
  Xt <- hbe %>%
    ...
    select(x, y) %>%
    as.matrix()
  tibble(id = id, t = tseq, msd = msd_fit(Xt = Xt, nlag = nlags))
}) %>% bind_rows()
...
aD_hat <- lapply(ids, function(id) {
  msd_id <- msd_hat %>%
  filter(id == !!id)
  msd_ls(msd = msd_id$msd, tseq = msd_id$t)
}) %>% bind_rows()

aD_hat %>%  
  ggplot(aes(x = D, y = alpha)) +
  geom_point() + 
  xlab(expression("D")) +
