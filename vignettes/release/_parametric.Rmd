...
# fit fbm to data
fbm_est <- fbm_fit(dX = dX, dt = dt, drift = "linear")
fbm_est
```

But what about estimates of the full set of parameters, $\tth = (\alpha, \mmu, \SSi)$?  We'll return to this a little bit later.
...
tibble(t = 1:Nmax * dt,
       ...
       fBM = exp(est$coef["logD"]) * (1:Nmax * dt)^est$coef["alpha"]) %>%
...
```{r fbmvsloc}
# fit fsd to data
fsd_est <- fsd_fit(dX = dX, dt = dt, drift = "linear")
# fit fma to data
fma_est <- farma_fit(dX = dX, dt = dt, order = c(0, 1), drift = "linear")
