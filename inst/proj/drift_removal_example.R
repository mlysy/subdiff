...
# for example we are interested in PEO5MD 1p52
dataID <- 15
...
# extract the movie-wise drift for movie 1
movieID <- 1
...
# original
est0 <- movie.est(paths, 1, movie_info, movie_drift, NL.method = FALSE)
# drift-removed
est1 <- movie.est(paths, 1, movie_info, movie_drift)

# plot the estimation results
xrng <- range(est1[,1], est0[,1])
yrng <- range(est1[,2], est0[,2])
plot(x = 1, type = "n", xlim = xrng, ylim = yrng, xlab = "alpha", ylab = "D", 
     main = "movie 1, 1p52", log = "y")
