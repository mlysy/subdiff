...
theta <- downsample.composite(Xt = Yt, dT = dT, Tz = Tz2, ds = ds)

comp.test3 <- function(theta) {
  suff <- itransFunc(theta, FALSE)
  Tz2$setAcf(fbm.acf(suff$alpha, ds*dT, N2))
  ans <- composite.full(Y = Yt, X = dT, suff$Beta, suff$Sigma, acf = Tz2, ds = ds)
  ans
}
...
ans1 <- downsample.composite(Xt = Yt, dT = dT, Tz = Tz2, ds = ds, varCal = TRUE, se = TRUE)
ans2 <- downsample.naive(Xt = Yt, dT = dT, Tz = Tz2, ds = ds, varCal = TRUE, se = TRUE)
ans3 <- downsample.avg1(Xt = Yt, dT = dT, Tz = Tz2, ds = ds, varCal = TRUE, se = TRUE)
