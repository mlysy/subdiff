...
Eigen::MatrixXd ls_var_cpp(double alpha, Eigen::VectorXd tau, int N) {
  ...
  Vt.array() = t1ov2.array().abs().pow(alpha);
  Vt.array() += Vt.transpose().array() - (t1ov2 - t2ov1).array().abs().pow(alpha);
  V.array() = Vt.array().square();
  // std::cout << "V = \n" << V << std::endl;
  for(int ii=1; ii<=N-1; ii++) {
    ...
    Vt.array() = (ii * tprod + t1ov2).array().abs().pow(alpha) - 
      // std::cout << "Vt = \n" << Vt << std::endl;
      // Vt.array() -=
      (ii * tprod + t1ov2 - t2ov1).array().abs().pow(alpha) - 
      // std::cout << "Vt = \n" << Vt << std::endl;
      // Vt.array() -=
      (ii * tprod).array().abs().pow(alpha) + 
      // std::cout << "Vt = \n" << Vt << std::endl;
      // Vt.array() +=
      (ii * tprod - t2ov1).array().abs().pow(alpha);
    // std::cout << "Vt = \n" << Vt << std::endl;
    Vt.array() = Vt.array().square();
    // std::cout << "Vt = \n" << Vt << std::endl;
    V.array() += (1.0-abs(1.0 * ii)/N) * (Vt + Vt.transpose()).array();
    // std::cout << "V = \n" << V << std::endl;
  }
  // std::cout << "V = \n" << V << std::endl;
  V = V.selfadjointView<Eigen::Lower>(); // copy lower half to upper half
  V *= .5/N;
  return V;
}
