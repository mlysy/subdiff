...
test_that("MLE is at the mode of the projection plots.", {
  ...
  replicate(n = ntest, {
    ...
    tau <- runif(1, 0, 1)
    sig <- runif(1, 0, 0.05)
    penalty <- as.logical(sample(0:1, 1))
    ndims <- sample(1:2, 1)
    acf1 <- fdyn_acf(alpha, tau, dT, N)
    ...
    # fdl
    theta_hat <- fdl_fit(dX, dT, var_calc = FALSE, penalty = penalty,
...
# fdy
# {
#   theta_hat1 <- fdl_fit1(dX, dT, sigma2 = sig^2, var_calc = FALSE, penalty = penalty,
#                          control = list(maxit = 1e6, trace = 0)) # fit MLE
#   # projection plots
#   Tz <- Toeplitz(n = N) # memory allocation
#   ocheck <- optim_proj(xsol = theta_hat1,
#                        fun = function(theta) loglik1(theta, sig^2, dX, dT, Tz, penalty),
#                        plot = TRUE, xrng = .1, npts = 20, equalize = FALSE)
#   expect_lt(max.xdiff(ocheck), .05)
# }
# 
# # flo
# {
#   theta_hat2 <- fdl_fit1(dX, dT, tau = tau, var_calc = FALSE, penalty = penalty,
#                          control = list(maxit = 1e6, trace = 0)) # fit MLE
#   # projection plots
#   Tz <- Toeplitz(n = N) # memory allocation
#   ocheck <- optim_proj(xsol = theta_hat2,
...
# # fbm
# {
#   theta_hat3 <- fdl_fit1(dX, dT, tau = tau, sigma2 = sig^2, var_calc = FALSE,
#                          control = list(maxit = 1e6, trace = 0)) # fit MLE
#   # projection plots
#   Tz <- Toeplitz(n = N) # memory allocation
#   ocheck <- optim_proj(xsol = theta_hat3,
