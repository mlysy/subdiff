...
#' @param C Optional vector of pre-computed OU scale coefficients.  Can't pass \code{C} without \code{r}.  When both \code{r} and \code{C} are supplied the function does nothign.  This is mainly for the convenience of calling \code{prony.coeff(...)} from other functions.
...
#' @export
prony.coeff <- function(lambda, r, C, nsteps = 100, tol = 0) {
  hasr <- !missing(r)
  hasC <- !missing(C)
  if(hasC) {
    if(!hasr) {
