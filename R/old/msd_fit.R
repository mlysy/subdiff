...
#' @param dT Optional interobservation time.  If supplied, estimates subdiffusion parameters `(alpha, D)` by regressing MSD over time on log-log scale.
#' @param logw Logical; whether or not regression should be log-weighted (see Details).
#' @param msd Optionally pre-computed sample MSD, in which case it is used for the regression estimate of `(alpha, D)`.
#' @return Sample MSD vector of length `nlag`, or list with elements `msd`, `alpha`, `D`.
#' @details When `dT` is supplied, a power-law fit to the empirical MSD is calculated by performing the linear regression
#' \preformatted{
